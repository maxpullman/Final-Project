...
public:
	...
	vector<vector<int>> AltState;  //Grid as State representation on its own
	...
	void create_grid() {
		...
		//Creating the x and y coordinate state representation
		for (int k = 0; k < ymax; k++) {
			for (int l = 0; l < xmax; l++) {
				States.push_back(l + k*(xmax));
			}
		}
		//Creating the Reward table for the grid class
...
int main() {
	...
	cout << "Agent has found goal." << endl;
